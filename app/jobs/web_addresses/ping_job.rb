module WebAddresses
  class PingJob < ApplicationJob
    rescue_from Faraday::Error do |exception|
      Results::ErrorProcessingInteraction.run!(web_address: @web_address, exception: exception)
    end
    ...
    def perform(web_address_id)
      ...
      Results::ResponseProcessingInteraction.run!(web_address: @web_address, response: response)
    end
  end
end
